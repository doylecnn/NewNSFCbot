...
func cmdOpenIslandQueue(message *tgbotapi.Message) (replyMessage []*tgbotapi.MessageConfig, err error) {
	...
	var shareBtn = tgbotapi.NewInlineKeyboardButtonSwitch("分享队列："+island.Name, "/join "+queue.ID)
	var dismissBtn = tgbotapi.NewInlineKeyboardButtonData("解散队列", "/dismiss "+queue.ID)
	var replyMarkup = tgbotapi.NewInlineKeyboardMarkup(tgbotapi.NewInlineKeyboardRow(shareBtn, dismissBtn))
	var replyText = fmt.Sprintf("队列已创建成功，队列ID：*%s*\n请使用分享按钮选择要分享排队的群/朋友\n/next 指令将向队列中下一顺位的朋友发送登岛密码\n/dismiss 立即解散队列", queue.ID)
...
func cmdNextIslandQueue(message *tgbotapi.Message) (replyMessage []*tgbotapi.MessageConfig, err error) {
	...
	return []*tgbotapi.MessageConfig{{
		...
		Text: "已通知下一位访客"},
		{
			...
			Text: fmt.Sprintf("您前往 %s 的航程即将触发，密码为：%s。祝你平安归来。", queue.Name, queue.Password)}}, nil
}
